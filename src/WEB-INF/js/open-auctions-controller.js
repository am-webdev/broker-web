...
(function () {
	...
	de.sb.broker.OpenAuctionsController.prototype.displayForm = function (auction) {
		...
		if(auction) {
			...
			inputElements[0].value = new Date(auction.closureTimestamp - 30*24*60*60*1000).toLocaleString(TIMESTAMP_OPTIONS);
	...
	de.sb.broker.OpenAuctionsController.prototype.persistBid = function(object, auctionIdentity, bidValue){		
		...
		de.sb.util.AJAX.invoke("/services/auctions/" + auctionIdentity + "/bid", "GET", {"Accept": "application/json"}, null, this.sessionContext, function (request) {
			if (request.status === 200 || request.status === 204) {
				...
				de.sb.util.AJAX.invoke("/services/auctions/" + auctionIdentity + "/bid", "POST", header, bid.price, self.sessionContext, function (request) {
					self.display();
				});
			}
		});
